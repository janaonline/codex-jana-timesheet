...
**Architecture:**
- `app/globals.css` defines all semantic CSS custom properties: `--color-bg`, `--color-surface`, `--color-surface-raised`, `--color-border`, `--color-border-strong`, `--color-text`, `--color-text-subtle`, `--color-text-muted`, `--color-text-placeholder`, `--color-primary`, `--color-primary-hover`, `--color-primary-ring`, `--color-primary-text`, `--color-error-bg`, `--color-error-border`, `--color-error-text`. The `.dark` class overrides all tokens and also sets `color-scheme: dark` so browsers render native form control popups (select dropdowns, scrollbars, spinners) in dark mode. `:root` sets `color-scheme: light` to match. `--color-primary-text` is stone-950 in both themes — the amber fill does not change between modes, so the readable foreground must not change either. `globals.css` also defines `::selection` highlight colors (`#fde047` / `#111111` in light; `#fcd34d` / `#0c0a09` in dark) and a `button, input, textarea, select { font: inherit }` reset to ensure form controls inherit the page font.
- `@custom-variant dark (&:where(.dark, .dark *))` enables the standard `dark:` prefix throughout.
- The `a { color: inherit; text-decoration: none; }` base reset lives inside `@layer base` so that Tailwind utility classes (which are in `@layer utilities`) can override link colors. Unlayered CSS always beats layered CSS regardless of specificity — do not move this rule outside a layer.
...
- Inside any yellow/amber-filled element (`bg-(--color-primary)` or `bg-amber-300`), use `text-stone-950` — not `text-(--color-text)`, which resolves to near-white in dark mode. Do not use `text-(--color-primary-text)`: Tailwind v4 does not compile arbitrary CSS-variable classes for runtime tokens, so they produce no output rule.
- For native form controls (`<input type="month">`, date pickers, etc.) on dark surfaces, add `dark:[color-scheme:dark]` so the browser renders built-in widgets (calendar icon, scrollbars) in dark-mode colours. Note: the page-level `color-scheme: dark` (set on `.dark` in `globals.css`) is the primary mechanism that makes native `<select>` dropdown popups readable in dark mode — element-level `color-scheme` alone is insufficient for OS-rendered popup layers on some platforms.
- For `<option>` elements inside a `<select>` that sits on a dark surface, apply inline styles `{ colorScheme: "light", color: "CanvasText", backgroundColor: "Canvas" }`. System color keywords (`CanvasText`, `Canvas`) resolve correctly inside the OS popup layer, keeping list items legible regardless of the page `color-scheme`. The `<select>` element itself still gets `dark:[color-scheme:dark]` for its collapsed-state widget. See `calendarDayStateOptionStyle` in `components/timesheets/timesheet-editor.tsx` for the reference implementation.
- For semantic status colors (e.g. badge, toast, calendar cells) that require multiple hues, use explicit `dark:` prefix pairs (e.g. `bg-emerald-50 dark:bg-emerald-950`). The `dark:` prefix is also acceptable for interactive component states where no semantic token exists (e.g. inactive tab hover: `dark:bg-stone-800 dark:hover:bg-stone-700`). It is never acceptable for plain structural layout — use tokens there.
