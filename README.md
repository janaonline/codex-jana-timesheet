...
- For native form controls (`<input type="month">`, date pickers, etc.) on dark surfaces, add `dark:[color-scheme:dark]` so the browser renders built-in widgets (calendar icon, scrollbars) in light colours on the dark background. Note: the page-level `color-scheme: dark` (set on `.dark` in `globals.css`) is the primary mechanism that makes native `<select>` dropdown popups readable in dark mode — element-level `color-scheme` alone is insufficient for OS-rendered popup layers on some platforms.
